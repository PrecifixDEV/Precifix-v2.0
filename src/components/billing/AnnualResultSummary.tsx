...
// import { Sale } from '@/hooks/use-sales-data'; // Skipping hook import, defining interface locally
// import { OperationalCostPayment } from '@/types/costs'; // Skipping if not verifying types exists

interface AnnualResultSummaryProps {
    year: number;
}

// Interface definitions adjusted to being local if needed
...
interface OperationalCostPayment {
    id: string;
    due_date: string;
    paid_value: number;
}
...
export const AnnualResultSummary = ({ year }: AnnualResultSummaryProps) => {
    ...
    const { data: annualSales, isLoading: isLoadingSales, error: salesError } = useQuery<AnnualSaleData[]>({
        ...
        queryFn: async () => {
            ...
            const { data, error } = await supabase
                ...
                .eq('user_id', user.id)
                .in('status', ['finished', 'paid', 'closed']) // Guessing statuses, can accept 'accepted' too if relevant
                .gte('created_at', format(new Date(year, 0, 1), 'yyyy-MM-dd'))
                .lte('created_at', format(new Date(year, 11, 31), 'yyyy-MM-dd'));
            if (error) throw error;

            // Map service_orders to AnnualSaleData interface expected by component
            return (data || []).map(item => ({
                id: item.id,
                total_price: item.final_amount || 0,
                created_at: item.created_at || '',
                is_sale: true, // Implicitly true for service orders considered sales
                status: item.status
            })) as AnnualSaleData[];
        },
    });
    ...
    // Commenting out for now until table existence is confirmed, or user provides more info.
    // Using an empty array to prevent error if table is missing.
    /*
    const { data: annualOperationalCostPayments, isLoading: isLoadingOperationalCostPayments, error: operationalCostPaymentsError } = useQuery<OperationalCostPayment[]>({
      queryKey: ['annualOperationalCostPayments', year],
      queryFn: async () => {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return [];
        const { data, error } = await supabase
    ...
    */
    const annualOperationalCostPayments: OperationalCostPayment[] = [];
    const isLoadingOperationalCostPayments = false;
    const operationalCostPaymentsError = null;
    ...
    if (salesError || operationalCostPaymentsError) {
        toast.error("Erro ao carregar resultados anuais");
                                ...
                                <TableRow key={result.month}>
                                    ...
                                    <TableCell
                                        className={cn(
                                            ...
                                            result.netRevenue >= 0 ? "text-emerald-500" : "text-destructive" // Standardizing success color
                                        )}
                                    >
